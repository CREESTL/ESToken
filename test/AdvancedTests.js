...
contract("Exchange", async ([owner, alice, bob, carol]) => {
  ...
  describe("Match tests", async () => {
    ...
    it("should match correct and subadd fee depends on available sum", async () => {
      return;
      await this.usdt.transfer(this.exchange.address, new BN("1000000000000"), {
        from: owner,
      });
      // 1 u / 1 e = 1 e / 1 u (by exchange)
      let calc = await this.exchange._match(
        this.usdt.address,
        usdt("70000000"),
        this.estt.address,
        estt("70000000"),
      ...
      calc = await this.exchange._match(
        this.usdt.address,
        new BN("1000000"),
        this.estt.address,
        new BN("1000000"),
  ...
  describe("Orders tests", async () => {
    ...
    it("should instant buy ESTT by 1:1 price", async () => {
      ...
      await this.usdt.transferFrom(carol, alice, new BN("1000000"), {
        from: alice,
      });
      //usdt
      ...
      (
        await this.usdt.allowance(bob, this.exchange.address, { from: bob })
      ...
      const response = await this.exchange.trade(
        this.usdt.address,
        new BN("1000000"),
        this.estt.address,
        new BN("1000000"),
    ...
    it("should instant sell ESTT by 1:1 price", async () => {
      ...
      (await this.usdt.balanceOf(bob)).should.be.bignumber.equal(
        new BN("992000")
      );
      (await this.estt.balanceOf(bob)).should.be.bignumber.equal(new BN("0"));
      (
        await this.usdt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("8000"));
      (
        await this.estt.balanceOf(this.exchange.address)
    ...
    it("should remove bad opposite", async () => {
      ...
      await this.usdt.approve(this.exchange.address, new BN("0"), {
        from: carol,
      });
      await this.estt.transfer(bob, new BN("900000"), { from: owner });
      await this.estt.approve(this.exchange.address, new BN("900000"), {
        from: bob,
      });
      const response = await this.exchange.trade(
        this.estt.address,
      ...
      const order = await this.exchange.getOrderByUid(bobOrders[0], {
        from: bob,
      });
    ...
    it("should place few orders continue trade", async () => {
      ...
      await this.estt.approve(this.exchange.address, estt("5000000"), {
        from: bob,
      });
      await this.usdt.approve(this.exchange.address, usdt("700000000"), {
        from: owner,
      });
      (
        await this.usdt.balanceOf(this.exchange.address)
      ...
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(estt("70000000"));
      await this.exchange.trade(
        this.usdt.address,
        usdt("70000000"),
        this.estt.address,
      ...
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(estt("0"));
      let response;
      let amount1 = usdt("1");
      let amount2 = estt("1");
      let total1 = new BN("0");
      ...
      for (let i = 0; i < count; ++i) {
        ...
        response = await this.exchange.trade(
          this.estt.address,
          amount2,
          this.usdt.address,
          amount1,
  ...
  describe("Fee tests", async () => {
    ...
    it("should have correct fee estt/usdt 1/1", async () => {
      await this.usdt.transfer(this.exchange.address, new BN("1000000"), {
        from: owner,
      });
      ...
      (
        await this.estt.allowance(bob, this.exchange.address, { from: bob })
      ).should.be.bignumber.equal(new BN("1000000"));
      (await this.usdt.balanceOf(bob)).should.be.bignumber.equal(new BN("0"));
      (
        await this.estt.balanceOf(this.exchange.address)
      ...
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("70000000992000"));
    });
    ...
    it("should have correct fee estt/usdt 0.9/1.0 (needed <= opposite)", async () => {
      ...
      await this.estt.approve(this.exchange.address, new BN("907200"), {
        from: carol,
      });
      await this.exchange.trade(
        this.estt.address,
      ...
      (await this.usdt.balanceOf(bob)).should.be.bignumber.equal(new BN("0"));
      (await this.usdt.balanceOf(carol)).should.be.bignumber.equal(
        new BN("1000000")
      );
    ...
    it("should have correct fee estt/usdt 0.9/1.0 (needed > opposite)", async () => {
      ...
      await this.estt.approve(this.exchange.address, new BN("1814400"), {
        from: carol,
      });
      await this.exchange.trade(
        this.estt.address,
      ...
      (await this.estt.balanceOf(carol)).should.be.bignumber.equal(
        new BN("907200")
      );
    });
    ...
    it("should have correct fee usdt/estt 1.0/0.9 (needed > opposite)", async () => {
      ...
      await this.usdt.approve(this.exchange.address, new BN("2000000"), {
        from: bob,
      });
      (
        await this.usdt.allowance(bob, this.exchange.address, { from: bob })
  ...
  describe("Negative tests", async () => {
    it("should remove orders if allowance is low", async () => {
      ...
      await this.usdt.approve(this.exchange.address, new BN("5000000"), {
        from: alice,
      }); // 5 usdt
      await this.estt.approve(this.exchange.address, new BN("1000000"), {
        from: alice,
