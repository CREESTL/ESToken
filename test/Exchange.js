...
const assertEqual = (a, b) =>
  assert.isTrue(
    Object.is(a, b),
    `Expected ${a.toString()} to equal ${b.toString()}`
  );
...
contract("Exchange", async ([owner, alice, bob, carol]) => {
  beforeEach(async () => {
    ...
    this.exchange = await Exchange.new(this.estt.address, this.usdt.address, {
      from: owner,
    });
  ...
  it("should deploy with less than 6 mil gas", async () => {
    const receipt = await web3.eth.getTransactionReceipt(
      this.exchange.transactionHash
    );
    assert.isBelow(
      receipt.gasUsed,
      process.env.NETWORK == "soliditycoverage" ? 12000000 : 6000000
    );
  });
  ...
  describe("Basic trade tests", async () => {
    beforeEach(async () => {
      ...
      await this.usdt.approve(this.exchange.address, usdt("5"), {
        from: alice,
      }); // 5 usdt
      await this.estt.approve(this.exchange.address, estt("1"), {
        from: alice,
      }); // 1 estt
    });
    ...
    it("should revert if price is too low and not 1:1", async () => {
      ...
      (await this.exchange.minPrice()).should.be.bignumber.equal(
        new BN("2000000")
      ); // 2
      // usdt
      await this.exchange.trade(
  ...
  describe("Get methods tests", async () => {
    beforeEach(async () => {
      ...
      await this.estt.approve(this.exchange.address, new BN("1000000"), {
        from: alice,
      }); // 1 estt
      await this.exchange.trade(
        this.usdt.address,
    ...
    it("should set/get referral bonus", async () => {
      (await this.exchange.referralBonus()).should.be.bignumber.equal(
        new BN("1000500000000000000")
      ); // 1 + 0.05%

      await expectRevert(
        this.exchange.setReferralBonus(new BN("999999999999999999"), {
          from: owner,
        }),
        "negative referral bonus"
      );
      await expectRevert(
        this.exchange.setReferralBonus(new BN("1000000000000000000"), {
          from: alice,
        }),
        "Ownable: caller is not the owner"
      );

      await this.exchange.setReferralBonus(new BN("1000000000000000000"), {
        from: owner,
      });
      (await this.exchange.referralBonus()).should.be.bignumber.equal(
      ...
      await this.exchange.setReferralBonus(new BN("1002000000000000000"), {
        from: owner,
      });
      (await this.exchange.referralBonus()).should.be.bignumber.equal(
    ...
    it("should set/get exchange fee", async () => {
      (await this.exchange.exchangeFee()).should.be.bignumber.equal(
        new BN("1008000000000000000")
      ); // 1 + 0.8%

      await expectRevert(
        this.exchange.setExchangeFee(new BN("999999999999999999"), {
          from: owner,
        }),
        "negative exchange fee"
      );
      await expectRevert(
        this.exchange.setExchangeFee(new BN("1000000000000000000"), {
          from: alice,
        }),
        "Ownable: caller is not the owner"
      );

      await this.exchange.setExchangeFee(new BN("1000000000000000000"), {
        from: owner,
      });
      (await this.exchange.exchangeFee()).should.be.bignumber.equal(
        new BN("1000000000000000000")
      ...
      await this.exchange.setExchangeFee(new BN("1002000000000000000"), {
        from: owner,
      });
      (await this.exchange.exchangeFee()).should.be.bignumber.equal(
        new BN("1002000000000000000")
    ...
    it("should set/get min price", async () => {
      ...
      await expectRevert(
        this.exchange.setMinPrice(new BN("900000"), { from: owner }),
        "min possible price not in range [1, 9999]"
      );
      await expectRevert(
        this.exchange.setMinPrice(new BN("10000000000"), { from: owner }),
        "min possible price not in range [1, 9999]"
      );
      await expectRevert(
        this.exchange.setMinPrice(new BN("1000000"), { from: alice }),
        "Ownable: caller is not the owner"
      );
    ...
    it("should get prices", async () => {
      // usdt
      const price_usdt_estt_0 = await this.exchange.getNextPrice(
        this.usdt.address,
        0
      );
      price_usdt_estt_0.should.be.bignumber.equal(new BN("50000")); // 5 usdt for 1 estt
      const price_usdt_estt_1 = await this.exchange.getNextPrice(
        this.usdt.address,
        price_usdt_estt_0
      );
      price_usdt_estt_1.should.be.bignumber.equal(new BN("100000")); // 10 usdt for 1 estt
      const price_usdt_estt_2 = await this.exchange.getNextPrice(
        this.usdt.address,
        price_usdt_estt_1
      );
      price_usdt_estt_2.should.be.bignumber.equal(new BN("0"));
      // estt
      const price_estt_usdt_0 = await this.exchange.getNextPrice(
        this.estt.address,
        0
      );
      price_estt_usdt_0.should.be.bignumber.equal(usdt("50.1")); // 1 estt for 50.1 usdt
      const price_estt_usdt_1 = await this.exchange.getNextPrice(
        this.usdt.address,
        price_estt_usdt_0
      );
    ...
    it("should get user uids", async () => {
      ...
      const rest_alice_address = alice
        .toString()
        .substr(alice.toString().length - 8)
        .toLowerCase();
      const alice_order_estt_usdt = alice_orders[0].toJSON().toLowerCase();
      assertEqual(
        alice_order_estt_usdt.substr(alice_order_estt_usdt.length - 9),
        "1" + rest_alice_address
      );
      for (let i = 1; i < 5; ++i) {
        const alice_order_usdt_estt = alice_orders[i].toJSON().toLowerCase();
        assertEqual(
          alice_order_usdt_estt.substr(alice_order_usdt_estt.length - 9),
          "2" + rest_alice_address
        );
      }
    });
    ...
    it("should get orders params", async () => {
      ...
      price_usdt_estt_2.should.be.bignumber.equal(new BN("0"));
      const price_usdt_estt_0_uids = await this.exchange.getUidsByPrice(
        this.usdt.address,
        price_usdt_estt_0
      );
      assertEqual(price_usdt_estt_0_uids.length, 1);
      const order_usdt_estt_0 = await this.exchange.getOrderByUid(
        price_usdt_estt_0_uids[0]
      );
      ...
      const price_usdt_estt_1_uids = await this.exchange.getUidsByPrice(
        this.usdt.address,
        price_usdt_estt_1
      );
      assertEqual(price_usdt_estt_1_uids.length, 3);
      for (let i = 0; i < 3; ++i) {
        const order_usdt_estt = await this.exchange.getOrderByUid(
          price_usdt_estt_1_uids[i]
        );
      ...
      price_estt_usdt_1.should.be.bignumber.equal(new BN("0"));
      const price_estt_usdt_0_uids = await this.exchange.getUidsByPrice(
        this.estt.address,
        price_estt_usdt_0
      );
      assertEqual(price_estt_usdt_0_uids.length, 1);
      const order_usdt_estt_4 = await this.exchange.getOrderByUid(
        price_estt_usdt_0_uids[0]
      );
  ...
  describe("Orders tests", async () => {
    it("should instant buy ESTT by 1:1 price", async () => {
      ...
      (
        await this.usdt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("0"));
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("70000000000000"));
      await this.exchange.trade(
        this.usdt.address,
        new BN("1000000"),
        this.estt.address,
      ...
      (await this.estt.balanceOf(bob)).should.be.bignumber.equal(
        new BN("1000000")
      );
      (await this.usdt.balanceOf(bob)).should.be.bignumber.equal(usdt("0"));
      (
        await this.usdt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(usdt("1"));
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("69999999000000"));
    });
    ...
    it("should instant buy ESTT by 1:1 price, min price 2u/1e", async () => {
      ...
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("69999999500000"));
    });

    it("should instant sell ESTT by 1:1 price", async () => {
      // "before" block
      await this.usdt.transfer(bob, new BN("1000000"), { from: owner });
      await this.usdt.approve(this.exchange.address, new BN("1000000"), {
        from: bob,
      });
      await this.exchange.trade(
        this.usdt.address,
      ...
      (
        await this.usdt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("1000000"));
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("69999999000000"));
      await this.estt.approve(this.exchange.address, new BN("1000000"), {
      ...
      (
        await this.usdt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(usdt("0.008"));
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("69999999992000"));
    });
    ...
    it("should instant sell ESTT by 1:1 price, min price 2u/1e", async () => {
      ...
      (
        await this.estt.balanceOf(this.exchange.address)
      ).should.be.bignumber.equal(new BN("69999999996000"));
    });
  });
  ...
  describe("Orders tests", async () => {
    ...
    it("should check right uid", async () => {
      ...
      await expectRevert(
        this.exchange.cancel(web3.utils.toBN(custom_uid), { from: alice }),
        "wrong token type"
      );
    });
    ...
    it("should revert if user has not rights to cancel order", async () => {
      ...
      await expectRevert(
        this.exchange.cancel(alice_orders_0[0], { from: bob }),
    ...
    it("should cancel orders", async () => {
      ...
      await expectRevert(
        this.exchange.cancel(alice_orders_0[0], { from: alice }),
    ...
    it("should support piecemeal purchases", async () => {
      ...
      await this.estt.approve(this.exchange.address, new BN("2016129"), {
        from: bob,
      }); // 2 estt + 0.8% fee
      await this.exchange.trade(
        this.estt.address,
      ...
      await this.exchange.trade(
        ...
        ZERO_ADDRESS,
        { from: bob }
      ); // 200 estt (+0.8% for fee) -> 1.5 usdt
      (
        await this.estt.balanceOf(alice, { from: alice })
      ).should.be.bignumber.equal(new BN("2000000"));
      (await this.estt.balanceOf(bob, { from: bob })).should.be.bignumber.lt(
        new BN("110")
      );
      (
        await this.usdt.balanceOf(alice, { from: alice })
      ).should.be.bignumber.equal(new BN("0"));
      (await this.usdt.balanceOf(bob, { from: bob })).should.be.bignumber.equal(
        usdt("5")
      );
    });
  });
  ...
  describe("Referral tests", async () => {
    it("should add referral bonus", async () => {
      ...
      await this.usdt.approve(this.exchange.address, usdt("100"), {
        from: alice,
      });
      await this.exchange.trade(
        this.usdt.address,
      ...
      balance_alice_2.should.be.bignumber.lt(new BN("100170050"));
  ...
  after(async () => {
    await Exchange.deployed().then(async (instance) => {
      ...
      console.log(
        "    Exchange initialisation and constructor code in bytes =",
        sizeOfB - sizeOfD
      );
      const receipt = await web3.eth.getTransactionReceipt(
        this.exchange.transactionHash
