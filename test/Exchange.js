...
contract('Exchange', async ([owner, alice, bob]) => {
    ...
    describe('Basic trade tests', async () => {
        ...
        it('should place an orders', async () => {
            await this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('100000'), ZERO_ADDRESS, {from: alice}); // 1 usdt -> 0.1 estt
            await this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('100000'), ZERO_ADDRESS, {from: alice}); // 1 usdt -> 0.1 estt
            await this.exchange.trade(this.usdt.address, ether('2'), this.estt.address, new BN('100000'), ZERO_ADDRESS, {from: alice}); // 2 usdt -> 0.1 estt
            await this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('100000'), ZERO_ADDRESS, {from: alice}); // 1 usdt -> 0.1 estt
            await this.exchange.trade(this.estt.address, new BN('1000000'), this.usdt.address, ether('50.1'), ZERO_ADDRESS, {from: alice}); // 1 estt -> 50.1 usdt
        });

        it('should revert if not enough balance', async () => {
            // usdt
            await expectRevert(this.exchange.trade(this.usdt.address, ether('5.000000000000000001'), this.estt.address, new BN('100000'), ZERO_ADDRESS, {from: alice}), 'not enough balance');
            await this.exchange.trade(this.usdt.address, ether('5'), this.estt.address, new BN('1000000'), ZERO_ADDRESS, {from: alice}); // 5 usdt -> 1 estt
            await expectRevert(this.exchange.trade(this.usdt.address, ether('0.000001'), this.estt.address, new BN('1'), ZERO_ADDRESS, {from: alice}), 'not enough balance');
            // estt
            await expectRevert(this.exchange.trade(this.estt.address, new BN('1000001'), this.usdt.address, ether('10.1'), ZERO_ADDRESS, {from: alice}), 'not enough balance');
            await this.exchange.trade(this.estt.address, new BN('1000000'), this.usdt.address, ether('50.1'), ZERO_ADDRESS, {from: alice}); // 1 estt -> 50.1 usdt
            await expectRevert(this.exchange.trade(this.estt.address, new BN('1'), this.usdt.address, ether('10.1'), ZERO_ADDRESS, {from: alice}), 'not enough balance');
        });

        it('should revert if price is low', async () => {
            // usdt
            await expectRevert(this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('1000001'), ZERO_ADDRESS, {from: alice}), 'ESTT can\'t be cheaper USDT');
            // estt
           await expectRevert(this.exchange.trade(this.estt.address, new BN('100001'), this.usdt.address, ether('0.1'), ZERO_ADDRESS, {from: alice}), 'ESTT can\'t be cheaper USDT');
        });
    });
    ...
    describe('Tests of get methods', async () => {
        ...
        it('should show prices', async () => {
            ...
            price_estt_usdt_0.should.be.bignumber.equal(ether('50.1')); // 1 estt for 50.1 usdt
        ...
        it('should orders params', async () => {
            ...
            order_usdt_estt_0[2].should.be.bignumber.equal(ether('2'));
            ...
            for (let i = 0; i < 3; ++i) {
                ...
                order_usdt_estt[2].should.be.bignumber.equal(ether('1'));
            ...
            order_usdt_estt_4[3].should.be.bignumber.equal(ether('50.1'));
    ...
    describe('Orders tests', async () => {
        it('should instant buy ESTT by 1:1 price', async () => {
            //usdt
            await this.usdt.transfer(bob, ether('1'), { from: owner });
            await this.usdt.increaseAllowance(this.exchange.address, ether('1'), { from: bob });
            ...
            await this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('1000000'), ZERO_ADDRESS, { from: bob }); // 1 usdt -> 1 estt
            ...
            (await this.usdt.balanceOf(bob)).should.be.bignumber.equal(ether('0'));
            (await this.usdt.balanceOf(this.exchange.address)).should.be.bignumber.equal(ether('1'));
        ...
        it ('should instant sell ESTT by 1:1 price', async () => {
            ...
            (await this.usdt.balanceOf(bob)).should.be.bignumber.equal(ether('0'));
            await this.exchange.trade(this.estt.address, new BN('1000000'), this.usdt.address, ether('1'), ZERO_ADDRESS, { from: bob }); // 1 usdt -> 1 estt
            const bob_orders_1 = await this.exchange.getMyOrders({ from: bob });
            assertEqual(bob_orders_1.length, 0);
            (await this.usdt.balanceOf(bob)).should.be.bignumber.equal(ether('0.992'));
            (await this.estt.balanceOf(bob)).should.be.bignumber.equal(new BN('0'));
            (await this.usdt.balanceOf(this.exchange.address)).should.be.bignumber.equal(ether('0.008'));
    ...
    describe('Orders tests', async () => {
        beforeEach(async () => {
            await this.usdt.transfer(alice, ether('5'), { from: owner }); // 5 usdt
            await this.usdt.approve(this.exchange.address, ether('5'), { from: alice }); // 5 usdt
            await this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('500000'), ZERO_ADDRESS, { from: alice }); // 1 usdt -> 0.5 estt
            await this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('500000'), ZERO_ADDRESS, { from: alice }); // 1 usdt -> 0.5 estt
            await this.exchange.trade(this.usdt.address, ether('2'), this.estt.address, new BN('500000'), ZERO_ADDRESS, { from: alice }); // 2 usdt -> 0.5 estt
            await this.exchange.trade(this.usdt.address, ether('1'), this.estt.address, new BN('500000'), ZERO_ADDRESS, { from: alice }); // 1 usdt -> 0.5 estt
        });
        ...
        it('should check right uid', async () => {
            ...
            await expectRevert(this.exchange.cancel(web3.utils.toBN(custom_uid), { from: alice }), 'Wrong type token');
        });
        ...
        it('should cancel orders', async () => {
            ...
            await expectRevert(this.exchange.cancel(alice_orders_0[0], { from: alice }), 'Wrong id');
        ...
        it('should support piecemeal purchases', async () => {
            ...
            await this.exchange.trade(this.estt.address, new BN('400000'), this.usdt.address, ether('1'), ZERO_ADDRESS, { from: bob }); // 0.4 estt -> 1 usdt
            const price_usdt_estt_0 = await this.exchange.getNextPrice(this.usdt.address, 0);
            const price_usdt_estt_0_uids = await this.exchange.getUidsByPrice(this.usdt.address, price_usdt_estt_0);
            const order_usdt_estt_0 = await this.exchange.getOrderByUid(price_usdt_estt_0_uids[0]);
            order_usdt_estt_0[4].should.be.bignumber.equal(ether('1.5872')); // 1.6 - 0.8% exchange fee
            const alice_orders_0 = await this.exchange.getMyOrders({ from: alice });
            assertEqual(alice_orders_0.length, 4);
            await this.exchange.trade(this.estt.address, new BN('400000'), this.usdt.address, ether('1'), ZERO_ADDRESS, { from: bob }); // 0.4 estt -> 1 usdt
            const bob_orders_0 = await this.exchange.getMyOrders({ from: bob });
            ...
            await this.exchange.trade(this.estt.address, new BN('504032'), this.usdt.address, ether('1'), ZERO_ADDRESS, { from: bob }); // 0.5 estt (+0.8% for fee) -> 1 usdt
            const alice_orders_2 = await this.exchange.getMyOrders({ from: alice });
            assertEqual(alice_orders_2.length, 2);
            await this.exchange.trade(this.estt.address, new BN('1008065'), this.usdt.address, ether('1.5'), ZERO_ADDRESS, { from: bob }); // 200 estt (+0.8% for fee) -> 1.5 usdt
    ...
    describe('Tests of referral', async () => {
        it('should add referral bonus', async () => {
            ...
            await this.exchange.trade(this.usdt.address, ether('100'), this.estt.address, new BN('100000000'), bob, { from: alice }); // 100 usdt -> 100 estt
            ...
            balance_alice_2.should.be.bignumber.lt(new BN('100120030'));
            balance_alice_2.should.be.bignumber.gt(new BN('100100030'));
        });
    });
});