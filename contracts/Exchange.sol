...
contract Exchange is IExchange, Ownable {
    ...
    function getMyOrders() external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < lengthUSDT; ++i) {
            myOrderUids[i + lengthESTT] = _ledger[msg.sender][_USDTAddress]
                .orders[i]
                .uid;
        }
        return myOrderUids;
    ...
    function trade(
        ...
        _orderCheck(order);
        _ledger[msg.sender][src].reservedBalance = _ledger[msg.sender][src]
            .reservedBalance
            .add(srcAmount);
        // less than 10 wei
        if (_trade(order) > 10) {
    ...
    function continueTrade(uint256 uid) external {
        ...
        require(
            msg.sender == storageOrder.trader,
